...
const userGoogleAuthController = {
  ...
  async authenticate(req, res) {
    ...
    try {
      ...
      // Create a new user
      // With Gravatar as picture
      // Password here is just to fill the database password field
      const newUser = {
        email: payload.email,
        name: payload.name,
        picture: userPicture({ email: payload.email })
      };

      const [id] = await knex('users_data').insert(newUser).returning('id');
      // generate auth token
      ...
      return res
        .status(201)
        .json({ user: userPrivateData({ ...newUser, id }), token });
    } catch (error) {
